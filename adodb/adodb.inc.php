...
//	if (!defined('ADODB_DIR')) define('ADODB_DIR',dirname(__FILE__));
	...
	*/
	...
	/**
	 * Audit SQL
	 * Save all transactions in the audit system
	...
	function create_trail($sql) {
			...
			if (!in_array($arrayQuery[0], ["INSERT", "UPDATE", "DELETE"])) {
				return;
			}				
			...
			if (strcmp($table, "SGD_AUDITORIA") != 0 && $type != '') {
				$sql = str_replace("'", "^", $sql);
				$query = "INSERT INTO SGD_AUDITORIA (FECHA, USUA_DOC, IP, TIPO, TABLA, ISQL) VALUES ('" . time() . "', '" . $_SESSION['usua_doc'] . "', '$realip', '$type', '$table','" . date("Y-m-d H:i:s") . "> $sql')";
