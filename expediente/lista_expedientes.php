...
function mostrarExpediente(numExpediente,numRadicado){
  $.post( "./expediente/lista_expediente.php", {numExpediente:numExpediente, numRadicado:numRadicado},  function(data , status) {
     $( "#tab-"+numExpediente ).html( data );
  });
}
    ...
    <div class="tabs-left" height="100%">
        <ul class="nav nav-tabs tabs-left" id="demo-pill-nav" height="100%">
            ...
            foreach ($arrExpedientes as $numExpediente => $datosExp) {
                ...
                ?>
                <li class="<?= $datoss ?>" onClick="mostrarExpediente('<?=$numExpediente ?>', '<?=$verrad ?>');" >
                    <a  href="#tab-<?=$numExpediente ?>" data-toggle="tab">
                     <span class="badge bg-color-blue txt-color-white">*</span><?= $numExpediente ?>
                    </a>
               </li>
                <?php
                $iExp++;
