...
//consulta para extraer serie y sub-serie validar nueva codigficacion jj-sanchez

$Rquery = "Select sgd_srd_codigo,sgd_sbrd_codigo, sgd_id_serie, sgd_id_subserie from sgd_mrd_matrird where sgd_mrd_codigo in (select sgd_mrd_codigo from sgd_rdf_retdocf where radi_nume_radi = '$nurad' order by sgd_rdf_fech desc limit 1 )";
   $Rrs = $db->conn->Execute($Rquery);
   $R_codigo_serie = $Rrs->fields["SGD_SRD_CODIGO"];
   $R_codigo_subserie = $Rrs->fields["SGD_SBRD_CODIGO"];
   $idSerie = $Rrs->fields["SGD_ID_SERIE"];
   $idSubSerie = $Rrs->fields["SGD_ID_SUBSERIE"];
    ...
    <div>
				<table class="table table-bordered table-striped" style="width:850;" align=center>
					...
					if ($perm_crea_exp_todasdependencias == 'no'){$_condicion_dependencia = " and d.depe_codi = $dependenciaExp"; };
						...
						<tr>
								...
								<td>
										<small>
												<label class=select>
														...
														$querySerie = "select distinct ($sqlConcat) as detalle, s.sgd_id_serie, s.sgd_srd_descrip
								...
								or s.sgd_id_serie = $codserie 
								order by  s.sgd_srd_descrip, s.sgd_id_serie
							";
		...
		$querySub = "select distinct ($sqlConcat) as detalle, su.sgd_sbrd_codigo
			...
			where (cast(m.depe_codi as varchar(10)) = '$coddepe' or cast(m.depe_codi_aplica as varchar(100)) like '%$coddepe%' or cast(m.depe_codi as varchar(10))='$depDireccion')
				and m.sgd_id_serie = '$codserie'
				and su.sgd_id_serie = '$codserie'
